...
function loadSection(section) {
  ...
  const map = {
    ...
    levels: renderLevels, tags: renderTags, logs: renderLogs, store: renderStoreAdmin,
...
async function renderStoreAdmin(main) {
  main.innerHTML = '<div class="loading-center"><div class="spinner"></div></div>';
  let products = [], orders = [], shopier = {};
  try {
    [products, orders, shopier] = await Promise.all([
      adminApi('/store/products'),
      adminApi('/store/orders'),
      adminApi('/store/settings'),
    ]);
  } catch (e) {
    main.innerHTML = `<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>${escHtml(e.message)}</p></div>`;
    return;
  }
  ...
  const productCard = (p) => {
    ...
    return `<div class="card">
      <div class="card-header"><span><i class="fas ${p.product_key === 'vip' ? 'fa-gem' : p.product_key === 'plus' ? 'fa-plus-circle' : 'fa-shield-alt'}" style="color:var(--red2);margin-right:8px"></i>${escHtml(p.title)}</span>
        <span class="badge ${p.active && p.visible ? 'badge-green' : 'badge-red'}">${p.active && p.visible ? 'Satışta' : 'Gizli / Kapalı'}</span></div>
      <div class="card-body">
        <div class="form-group"><label>Ürün adı</label><input data-field="title" value="${escHtml(p.title)}"></div>
        <div class="form-group"><label>Açıklama</label><textarea data-field="description" rows="3">${escHtml(p.description || '')}</textarea></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
          <div class="form-group"><label>Satış fiyatı</label><input type="number" min="0.01" step="0.01" data-field="price" value="${Number(p.price).toFixed(2)}"></div>
          <div class="form-group"><label>Eski / indirim karşılaştırma fiyatı</label><input type="number" min="0.01" step="0.01" data-field="compare_at_price" value="${p.compare_at_price == null ? '' : Number(p.compare_at_price).toFixed(2)}"></div>
        </div>
        <div class="form-group"><label>Özellikler <span style="color:var(--text3)">(her satıra bir özellik)</span></label><textarea data-field="features" rows="4">${escHtml(features)}</textarea></div>
        <div style="display:flex;gap:18px;flex-wrap:wrap;margin:10px 0 16px">
          <label class="checkbox-label"><input type="checkbox" data-field="active" ${p.active ? 'checked' : ''}> Satışa açık</label>
          <label class="checkbox-label"><input type="checkbox" data-field="visible" ${p.visible ? 'checked' : ''}> Mağazada görünür</label>
        </div>
        <button class="btn btn-primary store-product-save" data-key="${escHtml(p.product_key)}"><i class="fas fa-save"></i> Ürünü Kaydet</button>
      </div>
    </div>`;
  };

  const statusLabel = { paid: 'Ödendi', pending: 'Bekliyor', failed: 'Başarısız', rejected: 'Reddedildi' };
  main.innerHTML = `<div class="adm-section-header">
      <div class="adm-section-title"><div class="icon-pill"><i class="fas fa-store"></i></div> Mağaza & Ödemeler</div>
      <span class="badge ${shopier.shopier_enabled ? 'badge-green' : 'badge-red'}">${shopier.shopier_enabled ? 'Shopier aktif' : 'Shopier pasif'}</span>
    </div>
    <div class="card" style="margin-bottom:18px">
      <div class="card-header"><span><i class="fas fa-plug" style="color:#86efac;margin-right:8px"></i>Shopier bağlantısı</span></div>
      <div class="card-body">
        <div style="font-size:12px;color:var(--text2);margin-bottom:14px">PAT ve webhook token sunucuda şifreli saklanır; mevcut değerler tekrar gösterilmez.</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="form-group"><label>Shopier Personal Access Token ${shopier.shopier_pat_configured ? '<span class="badge badge-green">kayıtlı</span>' : ''}</label><input id="shopier-pat" type="password" placeholder="${shopier.shopier_pat_configured ? 'Değiştirmek için yeni PAT girin' : 'PAT'}"></div>
          <div class="form-group"><label>Mağaza kodu / slug</label><input id="shopier-slug" value="${escHtml(shopier.shopier_shop_slug || '')}" placeholder="shop-slug"></div>
          <div class="form-group"><label>Webhook token ${shopier.shopier_webhook_token_configured ? '<span class="badge badge-green">kayıtlı</span>' : '(boşsa Shopier API oluşturur)'}</label><input id="shopier-webhook-token" type="password" placeholder="${shopier.shopier_webhook_token_configured ? 'Değiştirmek için yeni token girin' : 'Webhook token'}"></div>
          <div class="form-group"><label>Webhook URL</label><input id="shopier-webhook-url" value="${escHtml(shopier.shopier_webhook_url || '')}"></div>
          <div class="form-group"><label>Shopier ürün görseli URL</label><input id="shopier-image-url" value="${escHtml(shopier.shopier_image_url || '')}"></div>
        </div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <button class="btn btn-outline" id="shopier-save"><i class="fas fa-save"></i> Ayarları Kaydet</button>
          <button class="btn btn-green" id="shopier-activate"><i class="fas fa-check-circle"></i> Bağlantıyı Test Et ve Etkinleştir</button>
          <span id="shopier-msg" style="font-size:12px"></span>
        </div>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:16px;margin-bottom:22px">${products.map(productCard).join('')}</div>
    <div class="card">
      <div class="card-header"><span><i class="fas fa-receipt" style="color:#fbbf24;margin-right:8px"></i>Ödeme kayıtları</span><span style="font-size:12px;color:var(--text3)">${orders.length} kayıt</span></div>
      <div style="overflow-x:auto"><table class="adm-table"><thead><tr><th>Kullanıcı</th><th>Ürün</th><th>Tutar</th><th>Durum</th><th>Shopier sipariş</th><th>Tarih</th></tr></thead><tbody>
        ${orders.length ? orders.map(o => `<tr>
          <td><strong>${escHtml(o.username)}</strong><div style="font-size:11px;color:var(--text3)">${escHtml(o.email || '')}</div></td>
          <td>${escHtml(o.title)}</td><td>${Number(o.amount).toLocaleString('tr-TR',{minimumFractionDigits:2})} ${escHtml(o.currency || 'TRY')}</td>
          <td><span class="badge ${o.status === 'paid' ? 'badge-green' : o.status === 'pending' ? 'badge-yellow' : 'badge-red'}">${statusLabel[o.status] || escHtml(o.status)}</span></td>
          <td style="font-size:11px;color:var(--text3)">${escHtml(o.shopier_order_id || '-')}</td><td style="font-size:11px;color:var(--text3)">${formatDate(o.created_at)}</td>
        </tr>`).join('') : '<tr><td colspan="6" style="text-align:center;color:var(--text3);padding:30px">Henüz ödeme kaydı yok.</td></tr>'}
      </tbody></table></div>
    </div>`;

  $$('.store-product-save').forEach(btn => btn.addEventListener('click', async () => {
    ...
    const payload = {
      ...
      compare_at_price: val('compare_at_price').value,
      features: val('features').value.split('\n').map(x => x.trim()).filter(Boolean),
      active: val('active').checked,
      visible: val('visible').checked,
    };
    btn.disabled = true;
    try { await adminApi('/store/products/' + encodeURIComponent(btn.dataset.key), { method:'PUT', body:JSON.stringify(payload) }); toast('Ürün kaydedildi'); }
    catch (e) { toast(e.message, 'error'); }
    finally { btn.disabled = false; }
  }));

  const shopierPayload = () => ({
    shopier_pat: $('#shopier-pat').value.trim(),
    shopier_shop_slug: $('#shopier-slug').value.trim(),
    shopier_webhook_token: $('#shopier-webhook-token').value.trim(),
    shopier_webhook_url: $('#shopier-webhook-url').value.trim(),
    shopier_image_url: $('#shopier-image-url').value.trim(),
  });
  $('#shopier-save').addEventListener('click', async () => {
    const msg = $('#shopier-msg');
    try { await adminApi('/store/settings', { method:'POST', body:JSON.stringify(shopierPayload()) }); msg.style.color='#86efac'; msg.textContent='Ayarlar kaydedildi.'; }
    catch (e) { msg.style.color='var(--red2)'; msg.textContent=e.message; }
  });
  $('#shopier-activate').addEventListener('click', async () => {
    const msg = $('#shopier-msg');
    try {
      await adminApi('/store/settings', { method:'POST', body:JSON.stringify(shopierPayload()) });
      await adminApi('/store/activate', { method:'POST' });
      msg.style.color='#86efac'; msg.textContent='Shopier bağlantısı test edildi ve etkinleştirildi.';
      setTimeout(() => loadSection('store'), 700);
    } catch (e) { msg.style.color='var(--red2)'; msg.textContent=e.message; }
  });
}
