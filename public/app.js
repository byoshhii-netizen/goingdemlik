...
function renderLogin(app) {
  ...
  document.title = 'Giriş Yap — CigCig';
  app.innerHTML = `<div class="auth-outer">
    <div class="auth-glass">
      <div class="auth-logo-wrap">
        <img src="/cigcig.png" alt="CigCig" style="width:72px;height:72px;border-radius:50%;object-fit:cover;box-shadow:0 0 28px rgba(189,162,117,0.35);margin-bottom:14px;display:block;margin-left:auto;margin-right:auto" />
        <div class="auth-heading">CigCig</div>
        <div class="auth-sub">Hesabına giriş yap</div>
      </div>
      <div class="auth-field">
        <label>Kullanıcı Adı veya E-posta</label>
        <div class="input-wrap">
          <i class="fas fa-user"></i>
          <input type="text" id="login-id" placeholder="kullanıcı_adı veya e-posta" autocomplete="username" />
        </div>
      </div>
      <div class="auth-field">
        <label>Şifre</label>
        <div class="input-wrap">
          ...
          <button type="button" class="auth-pw-toggle" id="login-pw-toggle" tabindex="-1">
            <i class="fas fa-eye" id="login-pw-icon"></i>
          </button>
        </div>
      </div>
      <button class="auth-btn-main" id="login-btn">Giriş Yap</button>
      <div id="login-error" class="auth-error"></div>
      <div class="auth-switch">Hesabın yok mu? <a href="/kayit" data-link>Kayıt Ol</a></div>
    </div>
  </div>`;
      ...
      if (data.pending_delete) {
        ...
        app.innerHTML = `<div class="auth-outer">
          <div class="auth-glass" style="border-color:rgba(220,38,38,0.4)">
            ...
            <button class="auth-btn-main" id="cancel-delete-btn" style="margin-bottom:10px">
              Vazgeç, Hesabımı Geri Al
            </button>
            <button class="auth-btn-main" id="keep-delete-btn" style="background:rgba(220,38,38,0.15);color:#f87171;box-shadow:none">
              Hayır, Silinsin
            </button>
          </div>
        </div>`;
...
function renderRegister(app) {
  if (currentUser) { navigate('/'); return; }
  document.title = 'Kayıt Ol — CigCig';
  app.innerHTML = `<div class="auth-outer">
    <div class="auth-glass">
      <div class="auth-logo-wrap">
        <img src="/cigcig.png" alt="CigCig" style="width:72px;height:72px;border-radius:50%;object-fit:cover;box-shadow:0 0 28px rgba(189,162,117,0.35);margin-bottom:14px;display:block;margin-left:auto;margin-right:auto" />
        <div class="auth-heading">Hesap Oluştur</div>
        <div class="auth-sub">CigCig topluluğuna katıl</div>
      </div>
      <div class="auth-field">
        <label>Kullanıcı Adı</label>
        <div class="input-wrap">
          <i class="fas fa-user"></i>
          <input type="text" id="reg-username" placeholder="kullanıcı_adı" autocomplete="username" />
        </div>
      </div>
      <div class="auth-field">
        <label>E-posta</label>
      ...
      <div class="auth-field">
        ...
        <div class="input-wrap">
          <i class="fas fa-lock"></i>
          <input type="password" id="reg-pw" placeholder="En az 6 karakter" autocomplete="new-password" style="padding-right:44px" />
          <button type="button" class="auth-pw-toggle" id="reg-pw-toggle" tabindex="-1">
      ...
      <div class="auth-field">
        ...
        <div style="display:flex;gap:8px;margin-top:4px">
          <button type="button" id="acc-type-public" class="acc-type-btn acc-active" style="flex:1;padding:10px 8px;border-radius:10px;border:1.5px solid rgba(189,162,117,0.5);background:rgba(189,162,117,0.12);color:var(--text,#f0f0f5);font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.2s">
            🌐 Açık Hesap
          </button>
          <button type="button" id="acc-type-private" class="acc-type-btn" style="flex:1;padding:10px 8px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:var(--text-muted,#888);font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.2s">
            🔒 Gizli Hesap
          </button>
        </div>
        <input type="hidden" id="reg-account-type" value="public" />
        <p style="font-size:11px;color:var(--text-muted,#888);margin-top:6px;line-height:1.5">Açık: profilin herkese görünür. Gizli: yalnızca onaylı takipçilerin görebilir.</p>
      </div>
      <div class="auth-field" style="margin-bottom:8px">
        <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer">
          <input type="checkbox" id="reg-kvkk" style="margin-top:2px;flex-shrink:0" />
          <span style="font-size:12px;color:var(--text-secondary,#aaa)">KVKK aydınlatma metnini okudum ve kabul ediyorum. <button type="button" id="kvkk-btn" style="background:none;border:none;color:#BDA275;font-size:12px;cursor:pointer;padding:0;font-family:inherit">Metni oku</button></span>
        </label>
      </div>
      <button class="auth-btn-main" id="reg-btn">Kayıt Ol</button>
      <div id="reg-error" class="auth-error"></div>
      <div class="auth-switch">Zaten hesabın var mı? <a href="/giris" data-link>Giriş Yap</a></div>
    </div>
  </div>`;

  // Hesap türü toggle
  const pubBtn = $('#acc-type-public');
  const privBtn = $('#acc-type-private');
  const accTypeInput = $('#reg-account-type');
  const activeStyle = { border:'1.5px solid rgba(189,162,117,0.5)', background:'rgba(189,162,117,0.12)', color:'var(--text,#f0f0f5)' };
  const inactiveStyle = { border:'1.5px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.04)', color:'var(--text-muted,#888)' };
  pubBtn.addEventListener('click', () => {
    accTypeInput.value = 'public';
    Object.assign(pubBtn.style, activeStyle);
    Object.assign(privBtn.style, inactiveStyle);
  });
  privBtn.addEventListener('click', () => {
    accTypeInput.value = 'private';
    Object.assign(privBtn.style, activeStyle);
    Object.assign(pubBtn.style, inactiveStyle);
  });
  ...
  const doRegister = async () => {
    ...
    const account_type = $('#reg-account-type').value;
    ...
    try {
      const data = await api('/auth/register', { method: 'POST', body: JSON.stringify({ username, email, password, kvkk_accepted, account_type }) });
      currentToken = data.token; currentUser = data.user;
      localStorage.setItem('token', currentToken);
      updateNavUI(); toast('Hoş geldiniz, ' + currentUser.username + '!');
      navigate('/');
