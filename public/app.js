...
let activePhotoAudio = null;
let photoAudioObserver = null;
let siteName = 'CigCig';
...
function squareCoverFile(file, size = 1000) {
  ...
  return new Promise(resolve => {
    ...
    image.onload = () => {
      ...
      canvas.toBlob(blob => {
        ...
        resolve(blob ? new File([blob], (file.name || 'kapak').replace(/\.[^.]+$/, '') + '-kare.jpg', { type: 'image/jpeg' }) : file);
      }, 'image/jpeg', 0.9);
    };
...
function renderRoute(fullPath) {
  ...
  if (path.startsWith('/siir/') && segs.length === 2) return renderPoemDetail(app, segs[1]);
...
async function renderHome(app) {
  ...
  async function renderPoemsSection() {
    const html = `<div class="section"><div class="section-header"><div class="section-title"><div class="section-title-bar"></div>Son Şiirler</div><a href="/siirler" data-link class="btn btn-ghost btn-sm">Tümü <i class="fas fa-arrow-right"></i></a></div><div id="home-poems" class="poem-grid"></div></div>`;
    const container = $('#home-sections'); container.insertAdjacentHTML('beforeend', html);
    try {
      const poems = await api('/poems');
      const el = $('#home-poems');
      if (!poems.length) el.innerHTML = '<div class="empty-state"><i class="fas fa-pen-fancy"></i><p>Henüz şiir yok.</p></div>';
      else el.innerHTML = poems.slice(0, 6).map(poem => `
        <div class="poem-card" onclick="navigate('/siir/${escHtml(poem.slug)}')">
          <div class="poem-card-header">
            <div class="poem-card-title">${escHtml(poem.title)}</div>
            <div class="poem-card-author">${avatarImg(poem, 'avatar-sm')} ${userDisplayName(poem)}</div>
          </div>
          <div class="poem-card-preview">${escHtml(poem.content.substring(0, 160))}${poem.content.length > 160 ? '…' : ''}</div>
          <div class="poem-card-footer">
            <span><i class="fas fa-heart"></i> ${poem.like_count || 0}</span>
            <span><i class="fas fa-comment"></i> ${poem.comment_count || 0}</span>
          </div>
        </div>`).join('');
    } catch {}
  }
  ...
  app.innerHTML = `<div class="container page">
    <div class="forum-detail">
      ...
      <div class="forum-detail-header">
        ...
        <div class="forum-detail-title" data-title-position="after-author">${escHtml(forum.title)}</div>
...
function poemCardHTML(poem) {
  ...
  const shortPreview = preview.length > 150 ? preview.substring(0, 150) + '…' : preview;
  return `<div class="poem-card" onclick="navigate('/siir/${escHtml(poem.slug)}')">
    <div class="poem-card-header">
      <div class="poem-card-title">${escHtml(poem.title)}</div>
      <div class="poem-card-author">${avatarImg(poem, 'avatar-sm')} ${userDisplayName(poem)}</div>
    </div>
    <div class="poem-card-preview">${escHtml(shortPreview)}</div>
    <div class="poem-card-footer">
      <span><i class="fas fa-heart"></i> ${poem.like_count || 0}</span>
      <span><i class="fas fa-comment"></i> ${poem.comment_count || 0}</span>
    </div>
...
function showNewPoemModal(existing = null) {
  ...
  showModal(isEdit ? 'Şiiri Düzenle' : 'Yeni Şiir', `
    <div class="form-group">
      <label>Başlık</label>
      <input id="poem-title" type="text" value="${escHtml(existing ? existing.title : '')}" placeholder="Şiirin başlığı" />
    </div>
    <div class="form-group">
      <label>Şiir</label>
      <textarea id="poem-content" rows="10" placeholder="Şiirini buraya yaz...">${escHtml(existing ? existing.content : '')}</textarea>
    </div>
    <label class="checkbox-label" style="margin-bottom:16px">
      <input type="checkbox" id="poem-hidden" ${existing && existing.is_hidden ? 'checked' : ''} />
      Bu şiiri gizli paylaş (yalnızca siz ve yönetim görebilir)
    </label>
    <button class="btn btn-primary" id="poem-submit" style="width:100%">${isEdit ? 'Güncelle' : 'Paylaş'}</button>
    <div id="poem-error" class="form-error mt-4"></div>
  `);

  $('#poem-submit').addEventListener('click', async () => {
    const title = $('#poem-title').value.trim();
    const content = $('#poem-content').value.trim();
    const isHidden = $('#poem-hidden').checked;
    if (!title || !content) {
      $('#poem-error').textContent = 'Başlık ve şiir metni zorunlu.';
      return;
    }

    try {
      if (isEdit) {
        const updated = await api('/poem/' + existing.slug, { method: 'PUT', body: JSON.stringify({ title, content, is_hidden: isHidden }) });
        toast('Şiir güncellendi');
        hideModal();
        navigate('/siir/' + updated.slug);
      } else {
        const created = await api('/poems', { method: 'POST', body: JSON.stringify({ title, content, is_hidden: isHidden }) });
        toast('Şiir paylaşıldı');
        hideModal();
        navigate('/siir/' + created.slug);
      }
    } catch (error) {
      $('#poem-error').textContent = error.message;
    }
  });
}
...
async function renderPoemList(app) {
  ...
  updatePageMeta('Şiirler – ' + siteName, 'Topluluğun yazdığı şiirleri keşfet.', '');
  app.innerHTML = `
    <div class="container page">
      <div class="search-bar"><i class="fas fa-search"></i><input type="text" id="poem-search" placeholder="Şiir ara..." /></div>
      <div id="poems-grid" class="poem-grid"><div class="loading-center"><div class="spinner"></div></div></div>
    </div>`;

  let poems = [];
  try { poems = await api('/poems'); } catch {}
  renderPoemGrid(poems);

  $('#poem-search')?.addEventListener('input', e => {
    const q = e.target.value.toLowerCase();
    renderPoemGrid(poems.filter(poem =>
      (poem.title || '').toLowerCase().includes(q) ||
      (poem.content || '').toLowerCase().includes(q) ||
      (poem.username || '').toLowerCase().includes(q)
    ));
  });
}
...
function renderPoemGrid(poems) {
  ...
  if (!el) return;
  if (!poems.length) {
    el.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-pen-fancy"></i><p>Şiir bulunamadı.</p></div>';
    return;
  }
  el.innerHTML = poems.map(poem => poemCardHTML(poem)).join('');
}

async function renderPoemDetail(app, slug) {
  app.innerHTML = `<div class="container page"><div class="loading-center"><div class="spinner"></div></div></div>`;
  let data;
  try { data = await api('/poem/' + slug); } catch (error) {
    app.innerHTML = `<div class="container page"><div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>${escHtml(error.message || 'Şiir bulunamadı veya gizli.')}</p></div></div>`;
    return;
  }

  const { poem, comments = [] } = data;
  const isOwner = currentUser && currentUser.id === poem.user_id;
  document.title = poem.title + ' – ' + siteName;
  updatePageMeta(poem.title + ' – ' + siteName, poem.content.substring(0, 155), '');

  const commentsHtml = comments.length ? comments.map(comment => `
    <div class="comment-item" style="border:1px solid var(--border);border-radius:12px;padding:12px;background:var(--bg-card2)">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;width:100%">
        <div style="display:flex;align-items:center;gap:8px;min-width:0">
          ${avatarImg(comment, 'avatar-sm')}
          <div style="min-width:0">
            <div style="font-weight:700">${userDisplayName(comment)}</div>
            <div style="font-size:11px;color:var(--text-muted)">${formatDate(comment.created_at)}</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
          <button class="btn btn-ghost btn-xs poem-comment-like" data-comment-id="${comment.id}" data-liked="${comment.liked ? '1' : '0'}">
            <i class="${comment.liked ? 'fas' : 'far'} fa-heart"></i> ${comment.like_count || 0}
          </button>
          ${(currentUser && currentUser.id === comment.user_id) ? `<button class="btn btn-ghost btn-xs poem-comment-delete" data-comment-id="${comment.id}"><i class="fas fa-trash"></i></button>` : ''}
        </div>
      </div>
      <div style="white-space:pre-wrap;line-height:1.6;margin-top:10px;color:var(--text-primary)">${escHtml(comment.content)}</div>
    </div>
  `).join('') : '<div class="empty-state"><i class="fas fa-comment"></i><p>Henüz yorum yok.</p></div>';
    ...
    <div class="container page">
      <div class="card" style="padding:24px">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:20px">
          <div>
            <div style="font-size:12px;color:var(--text-muted);letter-spacing:1px;text-transform:uppercase;font-weight:700;margin-bottom:6px">Şiir</div>
            <h1 style="font-size:32px;line-height:1.2;margin:0">${escHtml(poem.title)}</h1>
            <div style="margin-top:12px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;color:var(--text-muted)">
              ${avatarImg(poem, 'avatar-sm')}
              <span>${userDisplayName(poem)}</span>
              <span>·</span>
              <span>${formatDate(poem.created_at)}</span>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${isOwner ? `<button class="btn btn-outline btn-sm poem-edit-btn"><i class="fas fa-edit"></i> Düzenle</button>` : ''}
            ${isOwner ? `<button class="btn btn-danger btn-sm poem-delete-btn"><i class="fas fa-trash"></i> Sil</button>` : ''}
          </div>
        </div>

        <div style="white-space:pre-wrap;line-height:1.9;font-size:16px;color:var(--text-primary);background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:14px;padding:20px;overflow-wrap:anywhere">${escHtml(poem.content).replace(/\n/g, '<br>')}</div>

        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:20px">
          <button class="btn btn-ghost btn-sm poem-like-btn" data-liked="${poem.liked ? '1' : '0'}">
            <i class="${poem.liked ? 'fas' : 'far'} fa-heart"></i>
            <span>Beğen (${poem.like_count || 0})</span>
          </button>
          <button class="btn btn-ghost btn-sm poem-share-btn"><i class="fas fa-share-alt"></i> Paylaş</button>
        </div>
      </div>

      <div class="card" style="padding:20px;margin-top:20px">
        <div style="font-size:18px;font-weight:700;margin-bottom:14px">Yorumlar</div>
        ${currentUser ? `
          <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:16px">
            <textarea id="poem-comment-input" rows="3" placeholder="Şiire yorum yaz..."></textarea>
            <button class="btn btn-primary" id="poem-comment-send">Gönder</button>
          </div>
        ` : '<div class="empty-state"><i class="fas fa-user"></i><p>Yorum yazmak için giriş yap.</p></div>'}
        <div id="poem-comments-list" style="display:flex;flex-direction:column;gap:12px">${commentsHtml}</div>
      </div>
    </div>
  `;
  ...
  $('#poem-like-btn')?.addEventListener('click', async () => {
    try {
      ...
      if (btn) {
        ...
        btn.querySelector('span').textContent = `Beğen (${result.like_count || 0})`;
      }
    } catch (error) {
  ...
  $('#poem-share-btn')?.addEventListener('click', async () => {
    const url = location.origin + '/siir/' + encodeURIComponent(slug);
    try {
      await navigator.clipboard.writeText(url);
      toast('Şiir bağlantısı kopyalandı');
    } catch {
      toast(url);
    }
  });

  $('#poem-comment-send')?.addEventListener('click', async () => {
    const input = $('#poem-comment-input');
    const content = input?.value.trim();
    if (!content) return;
    try {
      await api('/poem/' + slug + '/comments', { method: 'POST', body: JSON.stringify({ content }) });
      input.value = '';
      renderPoemDetail(app, slug);
    } catch (error) {
      toast(error.message, 'error');
    }
  });

  $('#poem-comments-list')?.addEventListener('click', async event => {
    const likeBtn = event.target.closest('.poem-comment-like');
    const deleteBtn = event.target.closest('.poem-comment-delete');
    if (likeBtn) {
      try {
        const result = await api('/poem/' + slug + '/comments/' + likeBtn.dataset.commentId + '/like', { method: 'POST' });
        likeBtn.dataset.liked = result.liked ? '1' : '0';
        likeBtn.innerHTML = `<i class="${result.liked ? 'fas' : 'far'} fa-heart"></i> ${result.like_count || 0}`;
      } catch (error) { toast(error.message, 'error'); }
      return;
    }
    if (deleteBtn) {
      ...
      try {
        await api('/poem/' + slug + '/comments/' + deleteBtn.dataset.commentId, { method: 'DELETE' });
        renderPoemDetail(app, slug);
      } catch (error) { toast(error.message, 'error'); }
    }
  });
  ...
  if (isOwner) {
    document.querySelector('.poem-edit-btn')?.addEventListener('click', () => showNewPoemModal(poem));
    document.querySelector('.poem-delete-btn')?.addEventListener('click', async () => {
      if (!confirm('Şiiri silmek istediğine emin misin?')) return;
      try {
...
async function renderGroupDetail(app, slug) {
  ...
  const approvalPending = !isMember && joinRequestStatus?.approval && joinRequestStatus.status === 'pending';
  ...
  const canManageChannels = !!currentUser && (isOwner || isMod);

  // Üye olmayan kullanıcılar için önizleme sayfası göster
  if (!isMember && !isOwner && !approvalPending) {
    ...
    $('#join-preview-btn')?.addEventListener('click', async () => {
      ...
      try {
        const result = await api('/group/' + slug + '/join', { method: 'POST' });
        toast(result.approval_required ? 'Katılım isteğiniz onaya gönderildi.' : 'Gruba katıldınız!');
        renderRoute(location.pathname);
      } catch (e) {
  ...
  app.innerHTML = `<div class="container page">
    ...
    <div class="group-detail-layout">
      <div>
        ${group.allow_chat ? `
          <div class="group-channel-workspace" id="group-channel-workspace">
            <aside class="group-channel-sidebar">
              <div class="group-channel-sidebar-header">
                <span><i class="fas fa-hashtag"></i> Kanallar</span>
                ${canManageChannels ? '<button type="button" class="channel-icon-btn" id="channel-create-btn" title="Kanal oluştur"><i class="fas fa-plus"></i></button>' : ''}
              </div>
              <div class="group-channel-list" id="group-channel-list"></div>
              ${canManageChannels ? '<button type="button" class="channel-approval-btn" id="channel-approval-btn"><i class="fas fa-user-check"></i><span>Onay sistemi</span></button>' : ''}
            </aside>
            <section class="group-channel-main">
              <header class="group-channel-header"><div><i class="fas fa-hashtag" id="active-channel-icon"></i><strong id="active-channel-name">Kanal</strong><small id="active-channel-description"></small></div><button type="button" class="channel-mobile-btn" id="channel-mobile-btn"><i class="fas fa-list"></i></button></header>
              <div class="group-channel-messages" id="group-channel-messages"><div class="loading-center"><div class="spinner"></div></div></div>
              <div class="group-channel-composer" id="group-channel-composer"><input id="group-channel-input" type="text" placeholder="Mesaj yaz..." /><button type="button" class="btn btn-primary btn-sm" id="group-channel-send"><i class="fas fa-paper-plane"></i></button></div>
            </section>
          </div>
          <div class="chat-container legacy-group-chat">
  ...
  const channelWorkspace = $('#group-channel-workspace');
  ...
  const loadChannelMessages = async channel => {
    ...
    $('#active-channel-icon').className = channel.icon || 'fas fa-hashtag';
    const canWrite = !!currentUser && (isOwner || isMod && Number(channel.moderators_can_write) === 1 || Number(channel.can_write) === 1);
    $('#group-channel-composer').style.display = canWrite ? 'flex' : 'none';
    ...
    } catch (error) { channelMessages.innerHTML = `<div class="empty-state"><i class="fas fa-lock"></i><p>${escHtml(error.message)}</p></div>`; }
  };
  const renderChannels = () => {
    const allowed = visibleChannels();
    if (!activeChannel || !allowed.some(channel => Number(channel.id) === Number(activeChannel.id))) activeChannel = allowed[0] || null;
    channelList.innerHTML = allowed.length ? allowed.map(channel => `<div class="group-channel-item${activeChannel && Number(activeChannel.id) === Number(channel.id) ? ' active' : ''}" data-channel-id="${channel.id}">
      <i class="${escHtml(channel.icon || 'fas fa-hashtag')}"></i><span>${escHtml(channel.name)}</span>${canManageChannels && !Number(channel.is_default) ? `<button type="button" class="channel-manage-btn" data-channel-manage="${channel.id}" title="Kanal ayarları"><i class="fas fa-ellipsis"></i></button>` : ''}
    </div>`).join('') : '<div class="empty-state" style="padding:18px 8px"><i class="fas fa-hashtag"></i><p>Kanal yok.</p></div>';
    loadChannelMessages(activeChannel);
  };
  const showChannelModal = (channel = null) => {
    const editing = !!channel;
    showModal(editing ? 'Kanalı düzenle' : 'Yeni kanal oluştur', `<div class="form-group"><label>Kanal adı</label><input id="channel-name-input" maxlength="60" value="${escHtml(channel?.name || '')}" placeholder="genel" /></div><div class="form-group"><label>Açıklama</label><input id="channel-description-input" maxlength="180" value="${escHtml(channel?.description || '')}" placeholder="Bu kanalın amacı" /></div><div class="form-group"><label>Görünürlük</label><select id="channel-visibility-input"><option value="all" ${channel?.visibility === 'all' ? 'selected' : ''}>Üyeler görebilir</option><option value="members" ${channel?.visibility === 'members' ? 'selected' : ''}>Sadece üyeler</option><option value="approval_only" ${channel?.visibility === 'approval_only' ? 'selected' : ''}>Onaylananlar</option></select></div><label class="checkbox-label"><input id="channel-history-input" type="checkbox" ${channel?.can_view_history !== 0 ? 'checked' : ''}> Üyeler geçmiş mesajları okuyabilsin</label><label class="checkbox-label"><input id="channel-write-input" type="checkbox" ${channel?.can_write !== 0 ? 'checked' : ''}> Üyeler bu kanala yazabilsin</label><label class="checkbox-label"><input id="channel-mod-manage-input" type="checkbox" ${channel?.moderators_can_manage === 1 ? 'checked' : ''}> Moderatörler kanal ayarlarını yönetebilsin</label><label class="checkbox-label"><input id="channel-mod-write-input" type="checkbox" ${channel?.moderators_can_write === 1 ? 'checked' : ''}> Moderatörler yazabilsin</label><div style="display:flex;gap:8px;margin-top:16px"><button type="button" class="btn btn-primary" id="channel-save-btn"><i class="fas fa-check"></i> Kaydet</button>${editing ? '<button type="button" class="btn btn-danger" id="channel-delete-btn"><i class="fas fa-trash"></i> Sil</button>' : ''}</div><div id="channel-form-error" class="form-error mt-4"></div>`);
    $('#channel-save-btn').onclick = async () => {
      const name = $('#channel-name-input').value.trim();
      if (!name) return $('#channel-form-error').textContent = 'Kanal adı gerekli.';
      const payload = { name, description: $('#channel-description-input').value.trim(), visibility: $('#channel-visibility-input').value, can_view_history: $('#channel-history-input').checked, can_write: $('#channel-write-input').checked, moderators_can_manage: $('#channel-mod-manage-input').checked, moderators_can_write: $('#channel-mod-write-input').checked };
      try { const saved = await api(editing ? `/group/${slug}/channel/${channel.id}` : `/group/${slug}/channels`, { method: editing ? 'PUT' : 'POST', body: JSON.stringify(payload) }); if (editing) channels = channels.map(item => Number(item.id) === Number(saved.id) ? { ...item, ...saved } : item); else channels.push(saved); activeChannel = saved; hideModal(); renderChannels(); toast(editing ? 'Kanal güncellendi.' : 'Kanal oluşturuldu.'); } catch (error) { $('#channel-form-error').textContent = error.message; }
    };
    $('#channel-delete-btn')?.addEventListener('click', async () => { if (!confirm('Bu kanal ve mesajları silinsin mi?')) return; try { await api(`/group/${slug}/channel/${channel.id}`, { method: 'DELETE' }); channels = channels.filter(item => Number(item.id) !== Number(channel.id)); activeChannel = channels[0] || null; hideModal(); renderChannels(); toast('Kanal silindi.'); } catch (error) { $('#channel-form-error').textContent = error.message; } });
  };
  ...
  $('#channel-approval-btn')?.addEventListener('click', async () => {
    try {
      const result = await api(`/group/${slug}/approval/toggle`, { method: 'POST' });
      toast(Number(result.is_enabled) ? 'Onay sistemi aktif edildi.' : 'Onay sistemi kapatıldı.');
      channels = await api(`/group/${slug}/channels`);
      renderChannels();
      if (!Number(result.is_enabled)) return;
      const requests = await api(`/group/${slug}/approval/requests`);
      showModal('Onay bekleyenler', `<div id="approval-request-list">${requests.length ? requests.map(request => `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)"><img src="${escHtml(request.avatar || '/icons/cigcig-192.png')}" style="width:34px;height:34px;border-radius:50%;object-fit:cover" alt="" /><strong style="flex:1">${escHtml(request.username || 'Kullanıcı')}</strong><button type="button" class="btn btn-primary btn-sm" data-approval-action="approve" data-request-id="${request.id}" title="Onayla"><i class="fas fa-check"></i></button><button type="button" class="btn btn-danger btn-sm" data-approval-action="reject" data-request-id="${request.id}" title="Reddet"><i class="fas fa-xmark"></i></button></div>`).join('') : '<div class="empty-state" style="padding:20px 8px"><i class="fas fa-user-check"></i><p>Bekleyen onay yok.</p></div>'}<div id="approval-form-error" class="form-error mt-4"></div>`);
      $('#approval-request-list')?.addEventListener('click', async event => {
        const button = event.target.closest('[data-approval-action]');
        if (!button) return;
        button.disabled = true;
        try {
          await api(`/group/${slug}/approval/respond/${button.dataset.requestId}`, { method: 'POST', body: JSON.stringify({ approved: button.dataset.approvalAction === 'approve' }) });
          button.closest('div[style*="border-bottom"]')?.remove();
          toast(button.dataset.approvalAction === 'approve' ? 'Üye onaylandı.' : 'Talep reddedildi.');
        } catch (error) { button.disabled = false; $('#approval-form-error').textContent = error.message; }
      });
    } catch (error) { toast(error.message, 'error'); }
  });
  renderChannels();
...
async function renderBadges(app) {
  ...
  const badgeRows = badges.length ? badges.map(badge => {
    const isLockedAdminBadge = badge.key === 'admin';
    return `
    <div class="badge-setting-row" data-badge-key="${escHtml(badge.key)}">
      ...
      <div class="badge-setting-copy">
        ...
        <small>${typeLabels[badge.type] || 'Rozet'} · <span class="badge-setting-status">${isLockedAdminBadge ? 'Zorunlu · Profilde görünüyor' : (badge.is_active ? 'Profilde görünüyor' : 'Profilde gizli')}</span></small>
      </div>
      <label class="badge-visibility-toggle${isLockedAdminBadge ? ' is-locked' : ''}" title="${isLockedAdminBadge ? 'Yetkili rozeti zorunludur' : (badge.is_active ? 'Profilde gizle' : 'Profilde göster')}">
        <input type="checkbox" data-badge-toggle="${escHtml(badge.key)}" ${badge.is_active ? 'checked' : ''} ${isLockedAdminBadge ? 'disabled' : ''} />
  ...
  app.innerHTML = `
    <div class="container page badges-page">
      ...
      <div class="card badges-settings-card">
        ...
        <div class="card-body">
      <p class="settings-help">Rozeti pasif yaptığında sadece profilindeki rozet satırından gizlenir; rozet hesabından silinmez. <strong>Yetkili rozeti hesap güvenliği nedeniyle zorunludur ve gizlenemez.</strong></p>
...
async function renderProfile(app, username) {
  ...
  const { user, forums, books, groups, poems = [], photos = [], videos = [], reals, songs, level, levels, book_page_count, badges = [], ad_panels = [] } = data;
  ...
  let profileTabOrder = ['forums', 'books', 'poems', 'photos', 'groups', 'videos', 'reals', 'saved', 'songs', 'badges'];
  try {
  ...
  app.innerHTML = `<div class="container page">
    ...
    <div class="tabs">${profileTabOrder.filter(tab => isOwn || tab !== 'saved').map(tab => ({ forums:'Forumlar', books:'Kitaplar', poems:'Şiirler', photos:'Medya', groups:'Gruplar', videos:'Videolar', reals:'Reals', saved:'Kaydedilenler', songs:'Müzikler', badges:'Rozetler' })[tab] ? `<button class="tab ${tab === profileTabOrder.find(item => isOwn || item !== 'saved') ? 'active' : ''}" data-tab="${tab}">${({ forums:'Forumlar', books:'Kitaplar', poems:'Şiirler', photos:'Medya', groups:'Gruplar', videos:'Videolar', reals:'Reals', saved:'Kaydedilenler', songs:'Müzikler', badges:'Rozetler' })[tab]}</button>` : '').join('')}</div>
    ...
    <div id="tab-poems" class="hidden">
      ${poems.length ? `<div class="poem-grid">${poems.map(poem => poemCardHTML(poem)).join('')}</div>` : '<div class="empty-state"><i class="fas fa-pen-fancy"></i><p>Şiir yok.</p></div>'}
    </div>
...
async function renderMusicDetail(app, slug) {
  ...
  app.innerHTML = `
    <div class="song-detail-page">
        ...
        <div class="song-detail-meta-col">
          ...
          <div class="song-detail-actions-row">
            ${isUploader ? `
              ...
              <button class="btn btn-outline btn-sm" id="song-remastered-btn"><i class="fas fa-wand-magic-sparkles"></i> Remastered ekle</button>
              <button class="btn btn-outline btn-sm" id="song-recommend-btn"><i class="fas fa-headphones"></i> Dinleme önerisi ekle</button>
      ...
      ${(song.recommendations || []).length ? `
        <section class="song-recommendations">
          <div class="song-lyrics-title"><i class="fas fa-headphones"></i> Şunu da dinleyebilirsiniz</div>
          <div class="song-recommendations-grid">${song.recommendations.map(item => `<a class="song-recommendation-card" href="/muzik/${escHtml(item.slug)}" data-link><div class="music-cover-wrap">${item.cover_url ? `<img src="${escHtml(item.cover_url)}" class="music-cover" />` : `<div class="music-cover music-cover-ph"><i class="fas fa-music"></i></div>`}</div><span><b>${escHtml(item.title)}</b><small>${escHtml(item.artist_name)}</small></span></a>`).join('')}</div>
        </section>` : ''}
  ...
  const remasteredToggle = document.getElementById('song-remastered-toggle');
  remasteredToggle?.addEventListener('click', () => {
    activeVariant = activeVariant === 'original' ? 'remastered' : 'original';
    audio.pause();
    audio.src = activeVariant === 'remastered' ? song.remastered_audio_url : song.audio_url;
    audio.load();
    halfCounted = false;
    remasteredToggle.innerHTML = activeVariant === 'remastered'
      ? '<i class="fas fa-music"></i> Orijinal versiyonu dinle'
      : '<i class="fas fa-wand-magic-sparkles"></i> Remastered versiyonu dinle';
    playBtn.innerHTML = '<i class="fas fa-play"></i>';
  });
  ...
  audio.addEventListener('timeupdate', () => {
    if (!halfCounted && audio.duration && (audio.currentTime / audio.duration) >= 0.5) {
      halfCounted = true;
      fetch('/api/songs/'+slug+'/play-half?variant=' + activeVariant, {method:'POST'}).catch(()=>{});
    }
  });
  ...
  if (editBtn) {
    editBtn.addEventListener('click', () => {
      ...
      document.getElementById('ue-save').addEventListener('click', async () => {
        ...
        const coverFile = document.getElementById('ue-cover')?.files[0]; if (coverFile) fd.append('cover', await squareCoverFile(coverFile));
  ...
  remasteredBtn?.addEventListener('click', () => {
    ...
    `);
    document.getElementById('remastered-save')?.addEventListener('click', async () => {
      const input = document.getElementById('remastered-audio');
      const msg = document.getElementById('remastered-msg');
      if (!input?.files[0]) { msg.textContent = 'Ses dosyası seçmelisin.'; return; }
      const button = document.getElementById('remastered-save');
      button.disabled = true; button.innerHTML = '<div class="spinner" style="width:14px;height:14px"></div> Yükleniyor...';
      try {
        const fd = new FormData(); fd.append('audio', input.files[0]);
        await apiForm('/songs/' + song.id + '/remastered', fd);
        hideModal(); toast('Remastered versiyon eklendi.'); navigate('/muzik/' + slug);
      } catch (error) {
        msg.textContent = error.message; button.disabled = false; button.innerHTML = '<i class="fas fa-upload"></i> Yükle';
      }
    });
  });

  const recommendBtn = document.getElementById('song-recommend-btn');
  recommendBtn?.addEventListener('click', async () => {
    let songs = [];
    try { songs = await api('/songs'); } catch (error) { toast(error.message, 'error'); return; }
    const existing = new Set((song.recommendations || []).map(item => String(item.id)));
    ...
    `);
    document.getElementById('recommend-save')?.addEventListener('click', async () => {
      const selected = document.getElementById('recommend-song')?.value;
      try {
        await api('/songs/' + song.id + '/recommendations', { method: 'POST', body: JSON.stringify({ song_id: selected }) });
        hideModal(); toast('Dinleme önerisi eklendi.'); navigate('/muzik/' + slug);
      } catch (error) { document.getElementById('recommend-msg').textContent = error.message; }
    });
  });
}
...
async function renderArtistPanel(app) {
  ...
  document.getElementById('song-upload-btn').addEventListener('click', async () => {
    ...
    if (isOwn) {
      ...
      const cover = document.getElementById('s-cover')?.files[0]; if(cover) fd.append('cover', await squareCoverFile(cover));
    } else {
      ...
      fd.append('audio', audio);
      const cover = document.getElementById('s-cover-o')?.files[0]; if(cover) fd.append('cover', await squareCoverFile(cover));
    }
...
async function renderShareSong(app) {
  ...
  app.innerHTML = `<div class="container page" style="max-width:680px;margin:0 auto">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="form-group">
          <label>Kapak Fotoğrafı (isteğe bağlı, 1:1 kare)</label>
  ...
  document.getElementById('ss-submit').addEventListener('click', async () => {
    ...
    fd.append('audio', audio);
    if (cover) fd.append('cover', await squareCoverFile(cover));
...
function bindPhotoFeed(feed) {
  feed.querySelectorAll('[data-photo-id]').forEach(card => {
    api('/photos/' + encodeURIComponent(card.dataset.photoId) + '/view', { method: 'POST' }).catch(() => {});
  });
