...
async function renderRealsFeed(app) {
  ...
  function renderItems() {
    listEl.innerHTML = orderedReals.map(r => `
      <div class="reals-item" data-id="${r.id}" data-slug="${escHtml(r.slug)}" data-video-url="${escHtml(r.video_url)}">
        <div class="reals-video-box">
          ...
          <div class="reals-top-controls">
            <button class="reals-icon-btn mute-btn" title="Sesi aç/kapat"><i class="fas fa-volume-mute"></i></button>
          </div>
          <button class="reals-icon-btn reals-close-btn" title="Reals'tan çık"><i class="fas fa-times"></i></button>
          <div class="reals-meta">
            <div class="reals-user">${avatarImg(r)} <span>${userDisplayName(r)}</span></div>
            <div class="reals-desc">${escHtml(r.description||'')}</div>
          </div>
          <div class="reals-right-actions">
            <button class="reals-action-btn like-btn" title="Beğen"><i class="fas fa-heart"></i><span>${r.like_count||0}</span></button>
            <button class="reals-action-btn comment-btn" title="Yorumlar"><i class="fas fa-comment"></i><span>${r.comment_count||0}</span></button>
            <button class="reals-action-btn resend-btn" title="Yeniden paylaş"><i class="fas fa-retweet"></i><span>Paylaş</span></button>
            <button class="reals-action-btn share-btn" title="Mesajla paylaş"><i class="fas fa-paper-plane"></i><span>Gönder</span></button>
            <a href="/reals/${escHtml(r.slug)}" data-link class="reals-action-btn" title="Detayı aç"><i class="fas fa-link"></i><span>Detay</span></a>
          </div>
        </div>
      </div>`).join('');
    ...
    listEl.querySelectorAll('.like-btn').forEach(btn => btn.addEventListener('click', async (e) => {
      e.stopPropagation(); const it = btn.closest('.reals-item'); const id = it.dataset.id; try { btn.disabled=true; await api(`/video/${id}/like`, { method:'POST' }); const span = btn.querySelector('span'); span.textContent = Number(span.textContent||0)+1; btn.classList.add('liked'); } catch(e){ toast(e.message,'error'); } finally { btn.disabled=false; }
    }));
