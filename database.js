...
async function initDb() {
  await query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_vmb_user_badges_badge ON vmb_user_badges(badge_id);
    ...
    CREATE TABLE IF NOT EXISTS vmb_members (
      ...
      last_file_access TIMESTAMP,
      last_folder_accessed BIGINT,
      notification_sent_at TIMESTAMP,
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE
    );
    CREATE INDEX IF NOT EXISTS idx_vmb_members_user ON vmb_members(user_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_vmb_folders_file ON vmb_folders(file_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_vmb_pages_folder ON vmb_pages(folder_id);

    CREATE TABLE IF NOT EXISTS vmb_access_logs (
      id BIGSERIAL PRIMARY KEY,
      user_id BIGINT NOT NULL,
      file_id BIGINT,
      folder_id BIGINT,
      ...
      FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY(file_id) REFERENCES vmb_files(id) ON DELETE SET NULL,
      FOREIGN KEY(folder_id) REFERENCES vmb_folders(id) ON DELETE SET NULL,
      FOREIGN KEY(page_id) REFERENCES vmb_pages(id) ON DELETE SET NULL
    );
    CREATE INDEX IF NOT EXISTS idx_vmb_access_logs_user ON vmb_access_logs(user_id);
    CREATE INDEX IF NOT EXISTS idx_vmb_access_logs_file ON vmb_access_logs(file_id);

    -- Insert default VMB badges
    INSERT INTO vmb_badges (name, icon, color, badge_type, is_system, is_deletable, description)
    VALUES
      ('VMB', 'fas fa-shield', '#fbbf24', 'member', 1, 0, 'Müdafaa Birliği Üyesi'),
      ('VMB Yönetim', 'fas fa-shield-alt', '#ef4444', 'admin', 1, 0, 'Müdafaa Birliği Yönetim Takımı')
